...
class GstStreamer:
    def __init__(self, host="192.168.0.128", port=10000, width=500, height=375, fps=15):
        self.width = width
        self.height = height
        self.fps = fps
...
if __name__ == "__main__":
    ...
    streamers = [GstStreamer(host="192.168.0.113", port=p, fps=15) for p in ports]

    npu = AiMF_NPU()

    def worker():
        while True:
            frame = main_queue.get()
            for s in streamers:
                s.push(frame)

    t1 = threading.Thread(target=uart_thread, daemon=True)
    t1.start()
    t2 = threading.Thread(target=npu.work, args=(streamers[0],), daemon=True)  # detection
    t2.start()
