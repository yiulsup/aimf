...
class AiMF_NPU():
    def __init__(self):
        ...
        # latest frame for Flask streaming
    ...
    def work(self):
        while True:
            ...
            if len(frame.shape) == 2:   ### ADDED
                frame = cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR)

            h, w, _ = frame.shape                                                       
            frame_resized = cv2.resize(frame, (416, 416))
            frame_rgb = cv2.cvtColor(frame_resized, cv2.COLOR_BGR2RGB).astype(np.float32)   ### ADDED cast
            frame_rgb = frame_rgb / 255.0   ### ADDED normalization [0,1]
            frame_rgb = np.expand_dims(frame_rgb, axis=0).astype(np.float32)  ### ensure float32
